...
if ! docker rm frontEND; then
	...
	docker run -d -p 100:80 -v /media/master/Data/\[Project\]\ Blibli/PlaybookUsed/Monitoring/Haproxy/haproxy.cfg:/usr/local/etc/haproxy/haproxy.cfg --name frontEND haproxy
else
